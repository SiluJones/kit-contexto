...
> Versão atual publicada: **v1.21.0**. Índice em ~519 KB. Teste: 17/17 nichos, 0 erros JS.
...
## ⚠️ Nota de fidelidade para o nosso próprio desenvolvimento (dogfooding)
O nosso `index.html` (~519 KB) é grande: dentro deste Projeto ele cai em **RAG ("modo de pesquisa")**, então pela busca eu vejo só fragmentos. **Para editar o index com segurança, ele precisa ser ANEXADO na conversa** (como foi nesta sessão) — nunca reconstruído a partir de fragmentos. Os `.md` são pequenos: se forem os únicos no Projeto, ficam em contexto (inteiros); com o index junto, tudo vira RAG. Detalhe completo em CONTEXT.md.
...
Desvio pedido pelo usuário antes do Custom Inteligente: resolver a lacuna sobre contexto/RAG/persistência. Pesquisada a documentação oficial (RAG automático por tamanho; conhecimento do Projeto vs. anexo; sincronização do GitHub manual) e a literatura de "context rot"/"lost in the middle". Implementada a **v1.21.0** (fidelidade/contexto + plano de transferência) — transversal no UPDATE_PROTOCOL + tela "Tokens & Fluxos" + correção do item impreciso. Registrados D-015 e i-N9. Validado 17/17, 0 erros. **Próximo passo: começar o Custom Inteligente** (etapa 1: importação com concatenação + dedup) — e, ao migrar, o usuário deve ANEXAR o index na conversa nova (não confiar só no Projeto, que está em RAG).
