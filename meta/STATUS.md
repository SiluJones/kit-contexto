...
1. **Redigir e embutir as diretrizes novas no CLAUDE.md gerado** (numa passada só, com re-validação 17/17): **(i-N18)** manifesto de achatamento — *"se houver `_MANIFEST.md`, consulte-o; sufixo `__pasta` = colisão; entregue pelo nome real"* (já vale para o nosso projeto); **(i-N19)** verificar o estado real antes de repetir pendência de STATUS velho (refino de P8 + nota no `UPDATE_PROTOCOL`); **(i-N20)** template de commit em 3 linhas + nota sobre `git add .`; **(i-N21)** gatilho de feedback → seção «Feedback para o Kit» no IDEAS do piloto; **(i-N22)** válvula de desvio registrado. *Redações de i-N21/i-N22 propostas em IDEIAS — **aguardam validação do usuário** antes da passada.*
...
**FlatDrop / `_MANIFEST.md` (2026-06-11):** o usuário achata o repo com o FlatDrop; em colisão, o nome plano ganha sufixo `__pasta`. **Consultar o `_MANIFEST.md`** (tabela caminho original → nome plano) para o nome/caminho real e **entregar sempre pelo nome real** — regra completa no CLAUDE.md (transferência).
