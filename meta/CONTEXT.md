...
- **Mount do Projeto:** `/mnt/project/` (somente-leitura aqui). **Alimentado por upload direto** dos arquivos no Projeto, NÃO pelo conector do GitHub (D-018) — que popula só o RAG. Pode estar VAZIO se só o conector do GitHub estiver ligado; chega achatado (sem subpastas).
- **Index e arquivos grandes:** com a ferramenta de código ligada eu leio o `index.html` inteiro do mount `/mnt/project/` (verificado, mesmo com o Projeto em RAG). Caminho confiável = **subir o index direto no Projeto** + ligar a ferramenta de código. Anexar só no chat comum (sem ferramenta) ou se o arquivo não estiver no Projeto/mount.
- **Scratchpad:** `/home/claude/` — onde se constrói/edita isolado e roda teste. O teste jsdom dos nichos roda de `/home/claude` com o index em disco (nesta v1.24.0: `validate.js` + `validate-switch.js` + `validate-compose.js` + `validate-conflict.js`, **18 nichos**).
