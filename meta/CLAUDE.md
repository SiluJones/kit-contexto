...
> **Mudanças nesta revisão (2026-06-11, só docs):** adicionada a **regra do manifesto FlatDrop** na seção de transferência — consultar `_MANIFEST.md` para nomes reais; sufixo `__pasta` = renomeação de colisão; entregar sempre pelo nome real, sem deixar entregas de mesmo nome se sobreporem. Nada removido.
...
- **Manifesto FlatDrop (`_MANIFEST.md`) — regra de nomes (2026-06-11):** o usuário achata o repo com o **FlatDrop** antes de subir ao Projeto (o mount não tem subpastas e nomes iguais se sobrepõem). O FlatDrop gera um **`_MANIFEST.md`** com a tabela *caminho original → nome na pasta*; em colisão, o nome plano ganha sufixo **`__pasta`** (separador `__`). Regras: **(1)** ao ver arquivo com `__` no nome — ou em qualquer dúvida de nome —, consultar o `_MANIFEST.md` para o nome/caminho **real**, nunca deduzir pelo nome plano; **(2)** referir-se aos arquivos e **entregá-los sempre pelo nome real** (sem sufixo), informando o caminho de destino no handoff (o manifesto também diz onde cada um mora: raiz, `meta\`, `logs\`…); **(3)** se DUAS entregas tiverem o mesmo nome real (pastas diferentes), não deixar uma sobrepor a outra em `outputs/` — desambiguar e dizer explicitamente onde cada uma vai.
